...
| `site <cmd> [args]`     | Website (jaredeberle.org) tasks from anywhere — pure dispatch to `~/git/website/scripts/`: `new`/`publish`/`images`/`preflight`/`push`/`serve`/`archive`/`sync-hugo`/`csp` |
